...
def launch_workers(total_days, start_seed, cores, script_path, time_limit_hours=None):
    ...
    for i in range(cores):
        ...
        cmd = [
            ...
            "--days", str(worker_days),
            "--start-seed", str(worker_seed)
        ]
        
        print(f"[{i+1}/{cores}] Launching Worker: Seed {worker_seed} for {worker_days} days...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--total-days", type=int, default=16, help="Total days to generate")
    parser.add_argument("--start-seed", type=int, default=8000, help="Starting seed")
    parser.add_argument("--cores", type=int, default=16, help="Number of parallel processes")
    parser.add_argument("--duration", type=float, default=None, help="Stop after N hours (e.g. 4.0)")
    args = parser.parse_args()
    
    launch_workers(args.total_days, args.start_seed, args.cores, "13_regime_factory.py", args.duration)
